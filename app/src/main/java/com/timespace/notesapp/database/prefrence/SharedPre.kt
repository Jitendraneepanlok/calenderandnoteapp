package com.timespace.notesapp.database.prefrence
...
class SharedPre private constructor(context: Context) {
  ...
  fun setIsFacebookLoggedIn(value: Boolean){
    ...
    var language: String
        get() = GetDataString(LANGUAGE)?:"en"
        set(language) {
            SetDataString(LANGUAGE, language)
        }
    var district: String
        get() = GetDataString(DISTRICT)?:"other"
        set(district) {
            SetDataString(DISTRICT, language)
        }
    var town: String
        get() = GetDataString(TOWN)?:"other"
        set(town) {
            SetDataString(TOWN, language)
        }
    ...
    private fun GetThemeString(key: String): String? {
        var cbValue: String? = null
        try {
            cbValue = getSharedPreferences(mContext).getString(key, "1")
        } catch (e: Exception) {
            e.printStackTrace()
        }
        return cbValue
    ...
    private fun LogoutPrefrences() {
        ...
        removePreferences(SCREEN, mContext)
    ...
    fun setThemeData(theme: String) {
        SetDataString(THEME_DATA, theme)
    }

    val themeData: String?
        get() = GetThemeString(THEME_DATA)
}